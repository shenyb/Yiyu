<template>
  <div class="chat-panel">
    ...
    <div class="input-area" @dragover.prevent="dragOver=true" @dragleave="dragOver=false" @drop.prevent="onDrop" :class="{'drag-over':dragOver}">
      ...
      <div class="input-row">
        <textarea v-model="inputText" placeholder="上传文件后，输入问题…" rows="1" @keydown.enter.exact="sendMessage" @input="autoResize" :disabled="!hasFile"></textarea>
        <button class="send-btn" :disabled="!inputText.trim()||!hasFile" @click="sendMessage">↵</button>
      </div>
...
function sendMessage() {
  ...
  addMsg('ai', '好的，正在查阅文件内容...⏳')
  setTimeout(() => {
    const last=messages.value[messages.value.length-1]
    last.content='根据文件内容，我理解您的问题是...（AI 回答会在这里展示）'
  }, 1000)
}
</script>
